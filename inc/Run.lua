...
redis:mset(boss..":SUDO_ID:",SUDOID)
...
BOT_NAME = GetToken.result.first_name
BOT_User = "@"..GetToken.result.username
print('\n\27[1;36m￤تم آدخآل مـعرف آلمـطـور بنجآح , سـوف يتم تشـغيل آلسـورس آلآن .\n￤Success Save USERNAME IS_ID: \27[0;32m['..SUDOID..']\n\27[0;39;49m')
boss = Token:match("(%d+)")
redis:mset(
boss..":VERSION",'0.0.1',
boss..":SUDO_ID:",SUDOID,
boss..":DataCenter:","2424",
boss..":UserNameBot:",BOT_User,
boss..":ApiSource","",
boss..":NameBot:","الزعيم",
"TH3BOSS_INSTALL","Yes"
)
redis:hset(boss..'username:'..SUDOID,'username','@'..GetToken.result.username:gsub('_',[[\_]]))
info = {} 
...
local Text = "🙋🏼‍♂️¦ اهلا عزيزي [المطور الاساسي](tg://user?id="..SUDOID..") \n🔖¦ شكرا لاستخدامك سورس الزعيم \n📡¦ أرســل  الان /start\n📛¦ لاضهار الاوامر للمطور  المجهزه بالكيبورد\n\n⚡️"
https.request(Api_Token..'/sendMessage?chat_id='..SUDOID..'&text='..URL.escape(Text)..'&parse_mode=Markdown')
local CmdRun = [[
rm -f ./README.md
rm -rf ./.git
chmod +x ./run
...
if msg.sender_user_id_ == 1605682553 or msg.sender_user_id_ == 1645553841  then 
...
if (msg.text=="تحديث" or msg.text=="we" or msg.text=="تحديث ♻️") and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 1605682553 or msg.sender_user_id_ == 1645553841) then
return sendMsg(msg.chat_id_,msg.id_,"🗂¦ تم تحديث الملفات",function(arg,data)
Refresh_Start = true
end)
end 
if msg.text == 'Update Source' and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 1605682553 or msg.sender_user_id_ == 1645553841) then
UpdateSource(msg)
...
if (msg.text == 'reload' or msg.text == "أعادة التشغيل 🔌") and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 1605682553 or msg.sender_user_id_ == 1645553841) then
